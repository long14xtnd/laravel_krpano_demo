        ...
        <main id="main-contents" class="col-xs-12 col-sm-12 col-md-8">
            <section id="content" class="test">
                <div class="clearfix wrap-content">
                                ...
                                @if ($movie->resolution != 5)
                                    @if ($episode_current_list_count > 0)
                                        <div class="bwa-content">
                                            ...
                                            <a href="{{ url('xem-phim/' . $movie->id . '-' . $movie->slug . '/tap-' . $episode_tapdau->episode) }}"
                                                class="bwac-btn">
                            ...
                            <div class="film-poster col-md-9">
                                ...
                                <ul class="list-info-group">
                                    <li class="list-info-group-item"><span>Trạng Thái</span> : <span class="quality">
                                        ...
                                        @endif
                                        <span
                                            onclick="addFavoriteMovie('{{ $movie->id }}','{{ $movie->title }}','{{ $movie->image }}','{{ $movie->resolution }}','{{ $movie->name_eng }}','{{ $movie->phude }}','{{ $movie->slug }}')"
                                            style="cursor: pointer"><i class="fa-regular fa-heart"></i></span>
                                    </li>
                                        ...
                                        @if ($episode_current_list_count > 0)
                                            @if ($movie->thuocphim == 'phimbo')
                                                @foreach ($episode as $key => $ep)
                                                    <a href="{{ url('xem-phim/' . $movie->id . '-' . $movie->slug . '/tap-' . $ep->episode) }}"
                                                        rel="tag">Tập {{ $ep->episode }}</a>
                                                @endforeach
                                            @elseif($movie->thuocphim == 'phimle')
                                                @foreach ($episode as $key => $ep_le)
                                                    <a href="{{ url('xem-phim/' . $movie->id . '-' . $movie->slug . '/tap-' . $ep_le->episode) }}"
                    ...
                    <div class="entry-content htmlwrap clearfix">
                        {{-- <div class="fb-comments" data-href="https://cellphones.com.vn/" data-width="100%" data-numposts="10"
                        ...
                        <div class="video-item halim-entry-box">
                            <article id="post-38424" class="item-content">
                                {{-- <div class="fb-comment-embed" data-href="{{ $current_url }}" data-width="500"></div> --}}
                                <div class="fb-comments" data-href="https://cellphones.com.vn/" data-width="100%"
                                    data-numposts="10" style="background:white">
                                </div>
                            </article>
                            {{-- <div class="fb-comments" data-href="https://cellphones.com.vn/" data-width="100%"
                                data-numposts="10" style="background:white">
                        ...
                        @foreach ($related as $key => $hot)
                            <article class="thumb grid-item post-38498">
                                <div class="halim-item">
                                    <a class="halim-thumb"
                                        href="{{ route('movie', ['slug' => $hot->slug, 'id' => $hot->id]) }}"
                                        title="{{ $hot->title }}">
